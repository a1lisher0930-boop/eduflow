...
export const HomeworkPage: React.FC<HomeworkPageProps> = ({ homeworkList, onToggleHomework }) => {
  ...
  const [contactTeacherOpen, setContactTeacherOpen] = useState(false);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-[#131b2e] p-6 sm:p-8 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            onClick={() => setContactTeacherOpen(true)}
      ...
      <Modal
        isOpen={contactTeacherOpen}
        onClose={() => setContactTeacherOpen(false)}
        title="O‘qituvchiga Xabar Yuborish"
      >
        {teacherSent ? (
          <div className="p-6 text-center space-y-2">
            <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto animate-bounce" />
            <h4 className="font-extrabold text-base text-slate-900 dark:text-white">
              Xabaringiz o‘qituvchiga yuborildi!
            </h4>
            <p className="text-xs text-slate-500">Tez orada javob xabarnomalar bo‘limida ko‘rinadi.</p>
          </div>
        ) : (
          <form onSubmit={handleSendTeacherMessage} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                Fanni tanlang:
              </label>
              <select className="w-full p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-slate-100 outline-none">
                <option value="Informatika">Informatika — Dilnoza Rahimova</option>
                <option value="Matematika">Matematika — Jamshid Karimov</option>
                <option value="Fizika">Fizika — Nigora Aliyeva</option>
                <option value="Ingliz tili">Ingliz tili — Sardor Usmonov</option>
              </select>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="Savolingiz yoki topshiriq bo‘yicha izohingizni yozing..."
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={() => setContactTeacherOpen(false)}
                className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
                Bekor qilish
              </button>
